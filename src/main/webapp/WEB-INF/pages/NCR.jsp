...
<div class="card mb-4 wow fadeIn" style="visibility: visible; animation-name: fadeIn;">
  ...
  <form:form role="form" action="saveNCR" method="post"  modelAttribute="schedule">
    <form:hidden path="ID"/>
     <div class="form-row">
          ...
          <form:select  class="form-control " path="ncr_dept_id" items="${listAuditAreas}" readonly="${schedule.ncr_status != 'NEW'&&schedule.ncr_status != 'Under Investigation'}" itemValue="audit_area_id" itemLabel="aUDIT_AREA_NAME" />
          </div>
          </div>
          <div class="col">
          <div class="form-group">
            <label class="control-label">Date  </label>
            <form:input maxlength="100" type="date" class="form-control" placeholder="Date" path="ncr_date" readonly="${schedule.ncr_status != 'NEW'&&schedule.ncr_status != 'Under Investigation'}"/>
          </div>
          </div>
          </div>
          <div class="form-row">
          ...
          <div class="form-group">
            ...
            <form:input maxlength="100" type="text" path="ncr_source" required="required" class="form-control" placeholder="this can be used for data analysis" readonly="${schedule.ncr_status != 'NEW'&&schedule.ncr_status != 'Under Investigation'}"/>
          </div>
          </div>
          <div class="col">
          <div class="form-group">
            <label class="control-label">Procedure ref</label>
        <form:input maxlength="100" type="text" path="ncr_other" required="required" class="form-control" placeholder="(for audit findings only or if applicable)" readonly="${schedule.ncr_status != 'NEW'&&schedule.ncr_status != 'Under Investigation'}"/>
          </div>
          </div>
    ...
    <form:textarea required="required" path="ncr_desc" class="form-control" placeholder="Describe the nonconfirmity..." readonly="${schedule.ncr_status != 'NEW'&&schedule.ncr_status != 'Under Investigation'}"></form:textarea>
  ...
  <div class="card">
    ...
    <form:textarea required="required" path="ncr_root_cause" class="form-control" placeholder="Describe the root cause of the nonconfirmity..." readonly="${schedule.ncr_status == 'CLOSED'||schedule.ncr_status == 'Under Revision'}"></form:textarea>
    </div>
  </div>
  <div class="card">
    <div class="card-header text-white bg-info">
    ...
    <form:textarea required="required"  path="ncr_corrective_action" class="form-control" placeholder="(Describe the activity to be carried-out to correct the nonconfirmity)" readonly="${schedule.ncr_status == 'CLOSED'||schedule.ncr_status == 'Under Revision'}"></form:textarea>
    <div class="form-group col-md-6">
            <label class="control-label">Agreed Completion Date :</label>
        <form:input maxlength="100" type="Date" path="ncr_completion_date" required="required" class="form-control" placeholder="Agreed Completion Date" readonly="${schedule.ncr_status == 'CLOSED'||schedule.ncr_status == 'Under Revision'}"/>
          </div>
    </div>
  </div>
    ...
    <div class="form-group col-md-6">
      ...
      <c:choose>
           ...
           <form:input class="btn btn-info" path="Operation" type="submit"  value="Save"/>
            ...
            <c:choose>
             <c:when test="${schedule.ncr_status == 'Under Investigation'  && schedule.ID>0 }">
          <form:input class="btn btn-info" path="Operation" type="submit"  value="Solved"/>
            </c:when>
            </c:choose>
            
            <c:choose>
             <c:when test="${schedule.ncr_status == 'Under Revision'  && schedule.ID>0 }">

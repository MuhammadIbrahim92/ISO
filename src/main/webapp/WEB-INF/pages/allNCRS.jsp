  ...
  <div class="modal-dialog modal-lg" role="document">
    ...
    <!--/.Content-->
		...
		<table id ="ScheduleTable" class="table table-striped table-bordered" style="width:80%">
            ...
            <tbody>
			<c:forEach var="ncr" items="${listNCR}">
				<tr>
                    <td>${ncr.ID}</td>
					<td>${ncr.ncr_dept_name}</td>
					<td>${ncr.ncr_date}</td>
					<td>${ncr.ncr_initiator}</td>
					<td>${ncr.ncr_other}</td>
					<c:choose>
             <c:when test="${ncr.ncr_schedule_id==0 }">
					<td></td>
            </c:when>
            </c:choose>
            <c:choose>
            <c:when test="${ncr.ncr_schedule_id>0 }">
					<td><a href="InternalAudit?id=${ncr.ncr_schedule_id}">${ncr.ncr_schedule_id}</a></td>
            </c:when>
            </c:choose>
					<td>${ncr.ncr_status}</td>
					<td>			
			<a href="EditNCR?id=${ncr.ID}">Edit</a>
